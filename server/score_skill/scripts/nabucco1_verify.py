...
for pi,(src,dst) in enumerate(zip(source.pages,target.pages)):
    ...
    output_ties=tie_predecessors(b);output_sounding={}
    ...
    for sa,sb in zip(a['staves'],b['staves']):
        ...
        kc={0:5,4:1,1:4}[initial_key(pi,si)]
        ...
        for c in b['chars']:
            ...
            if pi==0 and si==3 and c['x']>530:continue
            if pi==1 and si==5 and 220<c['x']<240:continue
        ...
        for n,orig,want in zip(ns,originals,expected):
            ...
            sharps='FCGD' if pi==1 and si==5 and n['x']>215.89 else 'FCGDA'[:kc]
            alt=state.get(n['pitch'],1 if pitchname(n['pitch'])[0] in sharps else 0)
            if n['id'] in acc:
                alt={'/accidentals.flat':-1,'/accidentals.natural':0,'/accidentals.sharp':1,'/accidentals.doublesharp':2}[acc[n['id']]['glyph']]
                state[n['pitch']]=alt
            elif n['id'] in output_ties:
                alt=output_sounding[output_ties[n['id']]]
            output_sounding[n['id']]=alt
            assert (n['id'] in output_ties)==(want['tie_predecessor'] is not None),(pi,si,'tie mismatch',n,want)
