...
def initial_key(pi,si):
    return 0 if pi==0 and si<4 else 4 if pi==0 or (pi==1 and si<=5) else 1

def key_at(pi,si,x):
    return 1 if pi==1 and si==5 and x>215.89 else initial_key(pi,si)

def tie_predecessors(d):
    """Read ties between successive equal-pitched notes from the PDF's actual curves."""
    ties={}
    for p in d['paths']:
        if p['paint']!='f' or not any(z[1]==b'c' for z in p['commands']):continue
        cs=p['commands'];start=next(z for z in cs if z[1]==b'm');end=next(z for z in cs if z[1]==b'c')
        a=start[2];ss=start[3];b=end[2];es=end[3]
        x0=float(a[0])*ss['sx']+ss['tx'];y0=float(a[1])*ss['sy']+ss['ty']
        x1=float(b[-2])*es['sx']+es['tx'];y1=float(b[-1])*es['sy']+es['ty']
        for s in d['staves']:
            ns=sorted([d['chars'][i] for i in s['notes']],key=lambda n:n['x'])
            if not ns:continue
            n0=min(ns,key=lambda n:abs(n['x']+n['width']/2-x0))
            n1=min(ns,key=lambda n:abs(n['x']+n['width']/2-x1))
            if n0['pitch']!=n1['pitch'] or ns.index(n1)!=ns.index(n0)+1:continue
            if max(abs(n0['x']+n0['width']/2-x0),abs(n1['x']+n1['width']/2-x1))>3.3:continue
            if not (1.8<abs(y0-n0['y'])<8 and 1.8<abs(y1-n1['y'])<8):continue
            ties[n1['id']]=n0['id']
    return ties
...
def make():
    ...
    for pi,page in enumerate(reader.pages):
        ...
        for s in staves:
            ...
            if pi==1 and si==5:
                cancels=[c for c in sc if c['glyph'].startswith('/accidentals') and 220<c['x']<240]
                assert len(cancels)==4;omit.update(c['id'] for c in cancels)
                extras.append(('key',s,221.35,4))
            ...
            for n in ns:
                ...
                key=key_at(pi,si,n['x'])
                a=acc.get(n['id'])
                alteration=state.get(n['pitch'],base_alter(n['pitch'],key))
                if a:
                    alteration={'/accidentals.flat':-1,'/accidentals.natural':0,'/accidentals.sharp':1}[a['glyph']]
                    state[n['pitch']]=alteration
                elif n['id'] in ties:
                    alteration=sounding[ties[n['id']]]
                sounding[n['id']]=alteration
                ...
                audit.append({'id':n['id'],'staff':si,'bar_index':bar,'x':n['x'],'source_step':n['pitch'],'source_alter':alteration,
                              ...
                              'tie_predecessor':ties.get(n['id'])})
                if a:
                    if target_alter==2:
                        omit.add(a['id']); extras.append(('double',s,a['x']+a['width']-DOUBLE_WIDTH,a['y']-s['step']))
                    else:
                        code=music_codes[{-1:'/accidentals.flat',0:'/accidentals.natural',1:'/accidentals.sharp'}[target_alter]]
            ...
            for c in sc:
                if c['glyph'].startswith('/flags.'):
                    ...
                    if pi==2 and si==12 and 334<c['x']<474:
                    ...
                    changes[c['id']]={'dy':-s['step']}
                elif s['bottom']-35<c['y']<s['bottom'] and (c['font']=='/R14' or (c['font']=='/R12' and c['glyph'] in ['f','p'])):
                    changes[c['id']]={'dy':-s['step']}
        ...
        for yy,cc in groups.items():
            ...
            at=word.find('Clarinette 1 en La')
            if at>=0:
        ...
        for p in paths:
            ...
            if typ=='other':
                ...
                elif len(cmdtypes)==2 and h<.005 and w>2 and abs(p['lw']-.996)<.05:
                    typ='ledger'
                ...
                elif len(cmdtypes)==2 and h<3.6 and w<40 and abs(p['lw']-.797)<.015:
                    typ='tuplet'
                elif len(cmdtypes)==2 and .1<h<7 and w>15 and abs(p['lw']-.498)<.02 and y1<s['bottom']:
                    typ='hairpin'
            ...
            if typ=='ledger':
                ...
                replacement[p['op']]=[]
                continue
            dy=-s['step'] if typ in ['stem','beam','slur','tuplet','hairpin'] else 0.
        ...
        for e in extras:
            ...
            if typ=='key':
                ...
                for i,step in enumerate([8,5,9,6,3][:count]):
                    ...
                    glyph(music_codes['/accidentals.sharp'],19.9253,xx,s['bottom']+step*s['step'])
            elif typ=='headerflat':glyph(music_codes['/accidentals.flat'],12,e[1],e[2])
            elif typ=='ledger':
        ...
        report.append({'page':pi+1,'note_count':len(audit),'original_glyph_counts':dict(Counter(c['glyph'] for c in chars)),
                       ...
                       'ledger_lines':sum(e[0]=='ledger' for e in extras),'staff_count':len(staves),'ties':ties})
    ...
    writer.add_metadata({'/Title':'NABUCCO - Sinfonia - Clarinette 1 en Si bemol','/Subject':'Clarinet in A part transposed down one semitone for clarinet in B-flat; original systems and page numbers retained.'})
