...
def test_gold_set_regression_generates_exact_reviewed_pdf(api, monkeypatch, tmp_path, name, notes, pages):
    ...
    progress = []
    output, summary, verification = bridge.try_skill_transposition(
        str(job / "input.pdf"), str(job), request,
        lambda *values: progress.append(values))
    expected = bridge.CASES[hashlib.sha256(source).hexdigest()]
    assert verification["status"] == "passed"
    assert summary["noteEvents"] == notes
    assert summary["outputPages"] == pages
    assert Path(output) == job / "output.pdf"
    assert hashlib.sha256((job / "output.pdf").read_bytes()).hexdigest() == expected["output"]
    assert [p[0] for p in progress] == ["transposing", "verifying"]
...
def test_production_job_path_has_no_gold_set_adapter(api):
    source = inspect.getsource(api.process_score_job)
    assert "try_skill_transposition" not in source
    assert "SCORE_ENABLE_REVIEWED_ADAPTER" not in source
